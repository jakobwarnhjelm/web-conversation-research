/**
 * SID-INJICERAD extraktor för Spår A (F-SNAP-3).
 *
 * VIKTIGT: `grabReadable` måste vara HELT SJÄLVSTÄNDIG. `chrome.scripting.executeScript`
 * serialiserar funktionen med .toString() och kör den i sidans kontext — den kan inte
 * referera importer eller yttre variabler. Därför är läsbarhetslogiken inlinad här
 * (medvetet duplicerad från app/src/lib/extractReadable.ts, som används av port/tester/
 * Electron där en riktig Document finns att skicka in).
 *
 * Returnerar { title, textHtml } så service workern slipper någon DOM (SW saknar DOMParser).
 */
